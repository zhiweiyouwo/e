...
public class NoticeServiceImpl {
    ...
    public Page<NoticeReaderEntity> queryReceivePage(
        ...
        Page<NoticeReaderEntity> page = noticeReaderRepository.findPage(
                "personnel.notice.queryReceivePage",
                new MapQueryParam(noticeQueryParam), pageable);
        return page;
    }
    ...
    public NoticeEntity read(String id) {
        ...
        NoticeReaderEntity noticeReader = noticeReaderRepository.findByNoticeIdAndReaderId(id,
                userSessionService.getSessionUser().getId());
        if (noticeReader != null) {
            if (!noticeReader.getSeen()) {
                noticeReader.setReaderTime(new Date());
                noticeReader.setSeen(Boolean.TRUE);
            ...
            if (idsArr != null) {
                for (String idd : idsArr) {
                    ...
                    if (noticeEntity != null) {
                        ...
                        List<NoticeReaderEntity> noticeReaders = noticeReaderRepository
                                .findByNoticeId(idd);
